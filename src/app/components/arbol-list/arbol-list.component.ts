...
export class ArbolListComponent {
  ...
  EmpleadoId:number=0;
  constructor(private router: Router, private http: RestsService,private route: ActivatedRoute, private  dialog: MatDialog
    ) {}
        ...
        this.http.deleteArbol(this.EmpleadoId, ArbolId).subscribe(data => {
          if (data.state === 200) {
            Swal.fire({
              ...
              // Navega a la misma vista para recargarla
              this.eliminarLoteDeDatos(ArbolId);
            });
          } else {
            Swal.fire({
              title: 'Error',
  ...
  eliminarLoteDeDatos(loteId: number) {
    ...
    this.filteredLotes = this.filteredLotes.filter(lote => lote.IdArbol !== loteId);
    console.log("eliminados")
  }
  openDialog(imgen:string): void {
    debugger;
